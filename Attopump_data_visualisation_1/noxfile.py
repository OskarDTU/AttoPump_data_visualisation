...
shutil.rmtree('src/Attopump_data_visualisation_1.egg-info', ignore_errors=True);
for f in glob.glob('src/Attopump_data_visualisation_1/*.so'): os.remove(f);
for f in glob.glob('src/Attopump_data_visualisation_1/*.c'): os.remove(f);
"""

nox.options.default_venv_backend = "uv"

package = "Attopump_data_visualisation_1"
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"ATTOPUMP_DATA_VISUALISATION_1_COMPILE_MYPYC": "1"})
